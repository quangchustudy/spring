...
function testCompositeCreated() {
    ...
    if ! assertCurl 200 "curl $AUTH -k https://$HOST/product-composite/$PROD_ID_REVS_RECS -s"
    then
        echo -n "FAIL"
        return 1
    fi
...
function recreateComposite() {
  ...
  assertCurl 202 "curl -X DELETE $AUTH -k https://$HOST/product-composite/${productId} -s"
  assertEqual 202 $(curl -X POST -s -k https://$HOST/product-composite -H "Content-Type: application/json" -H "Authorization: Bearer $ACCESS_TOKEN" --data "$composite" -w "%{http_code}")
}
...
function testCircuitBreaker() {
    ...
    for ((n=0; n<3; n++))
    do
        assertCurl 500 "curl -k https://$HOST/product-composite/$PROD_ID_REVS_RECS?delay=3 $AUTH -s"
    ...
    assertCurl 200 "curl -k https://$HOST/product-composite/$PROD_ID_REVS_RECS?delay=3 $AUTH -s"
    ...
    assertCurl 200 "curl -k https://$HOST/product-composite/$PROD_ID_REVS_RECS $AUTH -s"
    assertEqual "Fallback product$PROD_ID_REVS_RECS" "$(echo "$RESPONSE" | jq -r .name)"
...
ACCESS_TOKEN=$(curl -k https://writer:secret@$HOST/oauth2/token -d grant_type=client_credentials -s | jq .access_token -r)
...
assertCurl 200 "curl $AUTH -k https://$HOST/product-composite/$PROD_ID_REVS_RECS -s"
assertEqual $PROD_ID_REVS_RECS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 3 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 404 (Not Found) error is returned for a non-existing productId ($PROD_ID_NOT_FOUND)
assertCurl 404 "curl $AUTH -k https://$HOST/product-composite/$PROD_ID_NOT_FOUND -s"
...
assertCurl 200 "curl $AUTH -k https://$HOST/product-composite/$PROD_ID_NO_RECS -s"
...
assertCurl 200 "curl $AUTH -k https://$HOST/product-composite/$PROD_ID_NO_REVS -s"
assertEqual $PROD_ID_NO_REVS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 0 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl $AUTH -k https://$HOST/product-composite/-1 -s"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl $AUTH -k https://$HOST/product-composite/invalidProductId -s"
assertEqual "\"Type mismatch.\"" "$(echo $RESPONSE | jq .message)"

# Verify that a request without access token fails on 401, Unauthorized
assertCurl 401 "curl -k https://$HOST/product-composite/$PROD_ID_REVS_RECS -s"
...
assertCurl 200 "curl $READER_AUTH -k https://$HOST/product-composite/$PROD_ID_REVS_RECS -s"
assertCurl 403 "curl -X DELETE $READER_AUTH -k https://$HOST/product-composite/$PROD_ID_REVS_RECS -s"

# Verify access to Swagger and OpenAPI URLs
echo "Swagger/OpenAPI tests"
assertCurl 302 "curl -ks  https://$HOST/openapi/swagger-ui.html"
assertCurl 200 "curl -ksL https://$HOST/openapi/swagger-ui.html"
assertCurl 200 "curl -ks  https://$HOST/openapi/webjars/swagger-ui/index.html?configUrl=/openapi/v3/api-docs/swagger-config"
assertCurl 200 "curl -ks  https://$HOST/openapi/v3/api-docs"
assertEqual "3.0.1" "$(echo $RESPONSE | jq -r .openapi)"
assertCurl 200 "curl -ks  https://$HOST/openapi/v3/api-docs.yaml"
